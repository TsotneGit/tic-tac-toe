...
def animation(x, y):
    ...
    if not falling and not winner:
        ...
        messagebox.showerror(
            title="Connect four", message="You can't make a move until it's your turn"
        )

    else:
        wn.bgcolor("black")
        messagebox.showerror(
            title="Connect four", message="You can't make a move after player won"
        )
...
def show_winner(winner):
    ...
    t.pendown()
    t.goto(
        -350 + winner[1] * SPLITS + SPLITS2, 300 - (winner[0] + 1) * SPLITS + SPLITS2
    )
    wn.update()
    sleep(1)
    wn.clear()
    ...
    if winner != 0:
        text_t.write(
            ["Yellow", "Red"][board[winner[0]][winner[1]] == "r"] + " won!",
            font=("Courier", 40),
            align="center",
        )
    else:
